...
async function loadLocalData() {
  ...
  // Snapshot BEFORE details
  console.groupCollapsed("pokedexData - BEFORE details (snapshot)");
  console.log(JSON.parse(JSON.stringify(pokedexData)));
  console.groupEnd();

  await loadPokemonDetails(); // Rufe die Funktion zum Laden der Pokémon-Details auf

  // Snapshot AFTER details
  console.groupCollapsed("pokedexData - AFTER details (snapshot)");
  console.log(JSON.parse(JSON.stringify(pokedexData)));
  console.groupEnd();
}
...
async function loadPokemonDetails() {
  for (let i = 0; i < pokedexData.length; i++) {
    ...
    // kleiner Fortschritts-Log (Snapshot des einzelnen Elements)
    console.log(
      `loaded details for index ${i}:`,
      JSON.parse(JSON.stringify(pokedexData[i])),
    );
  }
